...
public class Main {
    public static void main(String[] args) throws IOException, ParseException, InterruptedException {
        ...
        DefaultScheduler defaultScheduler = (DefaultScheduler) applicationContext.getBean("defaultScheduler");
        Properties prop = new Properties();
        List<Task> tasks = null;
        if (args.length > 0) {
            switch (args[0]) {
                case "clear-tasks":
                    defaultScheduler.removeAllTask();
                    System.exit(0);
                    break;
                default:
                    ...
                    for (Task task : tasks) {
                        defaultScheduler.insertTask(task);
