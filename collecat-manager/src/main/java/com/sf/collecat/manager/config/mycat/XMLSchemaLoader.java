 ...
 *
 */
package com.sf.collecat.manager.config.mycat;

import com.sf.collecat.manager.config.mycat.model.*;
import com.sf.collecat.manager.config.mycat.model.rule.TableRuleConfig;
import com.sf.collecat.manager.config.mycat.util.ConfigException;
...
public class XMLSchemaLoader{
	...
	public static void main(String[] args) {
		...
		System.out.println(xmlSchemaLoader
                .getSchemas()
                .get("TESTDB")
                .getTables()
                .get("COMPANY")
                .getDataNodes());
        System.out.println(xmlSchemaLoader.getDataNodes().get("dn1").getDatabase());
        System.out.println(xmlSchemaLoader.getDataNodes().get("dn1").getDataHost());
        System.out.println(xmlSchemaLoader.getDataHosts().get("localhost1").getWriteHosts()[0].getUrl());
        System.out.println(xmlSchemaLoader.getDataHosts().get("localhost1").getWriteHosts()[0].getUser());
        System.out.println(xmlSchemaLoader.getDataHosts().get("localhost1").getWriteHosts()[0].getPassword());
    }
	...
	private DBHostConfig createDBHostConf(String dataHost, Element node,
		...
		int weight = "".equals(weightStr) ? 1 : Integer.valueOf(weightStr) ;
	...
	private void loadDataHosts(Element root) {
		...
		for (int i = 0, n = list.getLength(); i < n; ++i) {
			...
			int writeType = "".equals(writeTypStr) ? 1 : Integer.valueOf(writeTypStr);
			...
			long logTime = "".equals(logTimeStr) ? 1 : Long.valueOf(logTimeStr) ;
